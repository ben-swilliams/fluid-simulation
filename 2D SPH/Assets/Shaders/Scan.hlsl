static const uint SCAN_BLOCK_SIZE = 512;

groupshared uint temp[SCAN_BLOCK_SIZE];
RWStructuredBuffer<uint> BlockSums;
RWStructuredBuffer<uint> SuperBlockSums;
...
void ScanBlockSums(uint tid, uint numSums, uint superBlockIdx) {
    uint blockStart = superBlockIdx * SCAN_BLOCK_SIZE;
    uint idx1 = blockStart + 2 * tid;
    uint idx2 = idx1 + 1;
    ...
    if (tid == 0)
    {
        SuperBlockSums[superBlockIdx] = temp[SCAN_BLOCK_SIZE - 1];
        temp[SCAN_BLOCK_SIZE - 1] = 0;
    }
...
void ScanSuperBlockSums(uint tid, uint numSuperSums) {
    uint idx1 = 2 * tid;
    uint idx2 = idx1 + 1;
    temp[2 * tid] = (idx1 < numSuperSums) ? SuperBlockSums[idx1] : 0;
    temp[2 * tid + 1] = (idx2 < numSuperSums) ? SuperBlockSums[idx2] : 0;
    ...
    if (idx1 < numSuperSums) SuperBlockSums[idx1] = temp[2 * tid];
    if (idx2 < numSuperSums) SuperBlockSums[idx2] = temp[2 * tid + 1];
}

void AddSuperBlockSums(uint gid, uint numBlockSums) {
    if (gid >= numBlockSums) return;

    uint superBlockIdx = gid / SCAN_BLOCK_SIZE;
    if (superBlockIdx == 0) return;

    BlockSums[gid] += SuperBlockSums[superBlockIdx];
}